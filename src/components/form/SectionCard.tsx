...
export function SectionCard({
  ...
  const visibleFields = section.fields.filter((field) => visibility[field.id] !== false);
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 shadow-xl shadow-black/20 backdrop-blur">
      <div className="border-b border-slate-800 px-6 py-4">
        <p className="text-xs uppercase tracking-[0.2em] text-indigo-400">Sección</p>
      ...
      <div className="grid gap-4 px-6 py-6 md:grid-cols-12">
        {visibleFields.map((field) => (
          <div key={field.id} className={`${gridSpan(field.grid_width)} col-span-12`}>
            <FieldControl
              field={field}
